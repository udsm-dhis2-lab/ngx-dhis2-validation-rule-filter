...
export function generateValidationRuleGroupAPI(dataElementUID: string[]) {
    const sanitizedDataElementUID = dataElementUID || [];
    if (sanitizedDataElementUID.length !== 0) {
        const validationRuleParameters = ['id', 'name', 'periodType'];
        const validationRuleGroupParameters = ['id', 'name'];
